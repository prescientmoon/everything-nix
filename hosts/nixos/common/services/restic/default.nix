{ config, lib, ... }:
let
  cfg = config.satellite.restic;

  # TODO: move the key setup to some kind of one-shot service
...
# }}}
{
  options.satellite.restic = {
    enable = lib.mkEnableOption "satellite's restic integration" // {
      default = true;
    };
  };
